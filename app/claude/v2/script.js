(() => {
  ...
  if (!reduce && window.gsap && window.ScrollTrigger) {
      ...
      gsap.to(step, {
        ...
        scrollTrigger: {
          ...
          onUpdate: self => {
            ...
            // Cada círculo se llena progresivamente según
            // el avance global del recorrido.
            const start = index / total;
            const end = (index + 1) / total;

            let fill = (progress - start) / (end - start);
            fill = Math.max(0, Math.min(1, fill));

            step.style.setProperty("--fill", Math.round(fill * 100));
          }
        }
      });
    });
    ...
    gsap.to(".mobile-cta", {
      ...
      scrollTrigger: {
        trigger: ".hero", start: "bottom top", onEnter: () => document.getElementById("mobileCta")?.classList.add("is-visible"),
